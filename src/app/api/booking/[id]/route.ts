...
export async function DELETE(_req: Request, ctx: Ctx) {
  ...
  // Pemohon bisa cancel sendiri kalau status masih menunggu admin atau menunggu atasan
  ...
  if (me.role === "pemohon") {
    ...
    if (!cancelable.includes(booking.status)) {
      return NextResponse.json({ error: "Tidak bisa membatalkan booking yang sudah disetujui/ditolak" }, { status: 400 });
    }
  }
