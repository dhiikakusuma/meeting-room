import Link from "next/link";
import { ChevronRight } from "lucide-react";
import { prisma } from "@/lib/prisma";
import { StatusBadge } from "@/components/status-badge";
import { EmptyState } from "@/components/empty-state";
import { formatTanggal } from "@/lib/datetime";

export const dynamic = "force-dynamic";

type SearchParams = Promise<{ status?: string }>;

export default async function AtasanRiwayatPage({ searchParams }: { searchParams: SearchParams }) {
  const sp = await searchParams;
  const where: Record<string, unknown> = sp.status
    ? { status: sp.status }
    : { status: { in: ["DISETUJUI", "DITOLAK_ATASAN", "MENUNGGU_ATASAN"] } };

  const list = await prisma.booking.findMany({
    where,
    orderBy: [{ tanggal: "desc" }],
    take: 200,
    include: { ruangan: { select: { nama: true } } },
  });

  const filters: [string, string | null][] = [
    ["Semua", null],
    ["Menunggu", "MENUNGGU_ATASAN"],
    ["Disetujui", "DISETUJUI"],
    ["Ditolak", "DITOLAK_ATASAN"],
  ];

  return (
    <div className="px-5 sm:px-8 py-6 max-w-4xl">
      <h1 className="serif text-2xl font-semibold text-ink-900">Riwayat Persetujuan</h1>
      <p className="text-sm text-ink-500 mt-1">
        Pengajuan yang pernah Anda tinjau atau menunggu Anda.
      </p>
      <div className="mt-5 flex flex-wrap gap-2">
        {filters.map(([label, value]) => {
          const href = value ? `/atasan/riwayat?status=${value}` : "/atasan/riwayat";
          const active = sp.status === value || (!sp.status && !value);
          return (
            <Link
              key={label}
              href={href}
              className={`px-3 py-1.5 rounded-full text-[12px] border transition-colors ${
                active
                  ? "bg-ink-900 text-cream-50 border-ink-900"
                  : "bg-white text-ink-700 border-ink-200 hover:bg-cream-100"
              }`}
            >
              {label}
            </Link>
          );
        })}
      </div>

      <div className="mt-5">
        {list.length === 0 ? (
          <EmptyState title="Belum ada riwayat" />
        ) : (
          <div className="space-y-2">
            {list.map((b) => (
              <Link
                key={b.id}
                href={`/atasan/booking/${b.id}`}
                className="flex items-center gap-3 rounded-2xl border border-ink-100 bg-white p-4 hover:border-ink-200 transition-colors"
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <StatusBadge status={b.status} />
                    {b.nomorSurat && (
                      <span className="text-[10.5px] text-ink-400 nums">{b.nomorSurat}</span>
                    )}
                  </div>
                  <p className="serif text-sm font-semibold text-ink-900 truncate">{b.agenda}</p>
                  <p className="text-[11.5px] text-ink-500 mt-0.5 nums">
                    {b.pemohonNama} · {b.bidangNama}
                  </p>
                  <p className="text-[11.5px] text-ink-500 nums">
                    {formatTanggal(b.tanggal)} · {b.jamMulai}–{b.jamSelesai} · {b.ruangan.nama}
                  </p>
                </div>
                <ChevronRight className="h-4 w-4 text-ink-400" />
              </Link>
            ))}
          </div>
        )}
      </div>
    </div>
  );
}
